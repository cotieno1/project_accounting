...
@login_required
def _rfq_letter_context(request):
    """Build template context for RFQ letter screen view and PDF."""
    task_id = request.GET.get('task_id')
    supplier_ids_raw = request.GET.get('supplier_ids', '')
    ...
    return_url = request.GET.get('return') or (
        reverse("rfq_manager") + f"?task_id={active_task.project_id}"
    )
    context = {
        'task': active_task,
        'today': timezone.now(),
        ...
        'supplier_count': len(supplier_packets),
    }
    context.update(branding_template_context(request))
    return context
...
@login_required
def print_rfq_letter_pdf(request):
    """Native PDF for RFQ package (mobile save/share when no printer detected)."""
    ...
    if context['supplier_packets']:
        label = context['supplier_packets'][0]['ro_no'] or label
    try:
        pdf_bytes = build_pdf_bytes('rfq_print_letter.html', context)
    except ImportError:
        messages.error(request, "PDF support is not installed on the server.")
        return redirect(reverse('print_rfq_letter') + '?' + request.META.get('QUERY_STRING', ''))
...
def _misc_bom_picker_lines(task, mpo=None):
    ...
    for item in (
        ...
        if item.pk in used_ids:
            continue
        code = (item.source_package_code or "").strip() or "UNCATEGORIZED"
        rows.append(
            {
                "id": item.pk,
                "label": (
...
def _misc_bom_categories(task, mpo=None):
...
def _misc_bom_lines_for_packages(task, package_codes, mpo=None):
    """Remaining BOM lines for the selected package codes (category load)."""
    wanted = {str(c).strip() for c in (package_codes or []) if str(c).strip()}
    if not wanted:
        return []
    return [
        line
        for line in _misc_bom_picker_lines(task, mpo)
        if line["package_code"] in wanted
    ]


def _misc_session_bom_packages(request):
    raw = request.session.get("misc_bom_package_codes") or []
            ...
            with transaction.atomic():
                ...
                if "new_ro" in request.POST:
                    ...
                    if "select_supplier" in request.POST:
                        ...
                        bom_ids = [
                            str(v).strip()
                            for v in request.POST.getlist("bom_item_id")
                            if str(v).strip()
                        ]
                        if not bom_ids:
                            raise ValueError("Select at least one BOM line and enter its unit price.")
                        added = 0
                        for bom_item_id in bom_ids:
                            price_raw = (
                                request.POST.get(f"unit_price_{bom_item_id}")
                                or request.POST.get("unit_price")
                                or ""
                            ).strip()
                            if price_raw == "":
                                continue
                        ...
                        if added == 0:
                            raise ValueError(
                                "Enter a known unit price for at least one selected BOM line."
                            )
                        _recalc_mpo_total(mpo)
                        remaining = _misc_bom_lines_for_packages(
                            active_task,
                            _misc_session_bom_packages(request),
                            mpo,
                        )
                        if not remaining:
                            request.session.pop("misc_bom_package_codes", None)
                        ...
                        messages.success(
                            request,
                            f"Added {added} BOM line(s) with known price.",
                        )
